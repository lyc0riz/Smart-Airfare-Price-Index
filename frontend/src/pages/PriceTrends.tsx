...
import { ROUTES, AIRLINES, RANGE_OPTIONS, buildSeries, aggregate } from '../lib/prototype/apix-data'
...
export function PriceTrends() {
  ...
  const allIndia = useMemo(() => buildSeries('ALL', airlineCode, Math.max(days, 60)), [airlineCode, days])
  const routeSeries = useMemo(() => buildSeries(routeCode === 'ALL' ? 'DEL-BOM' : routeCode, airlineCode, Math.max(days, 60)), [routeCode, airlineCode, days])

  const mainData = useMemo(() => {
    const series = aggregate(allIndia, frequency)
