...
import { runBacktest, type BacktestResult, type ContributionRow } from '../lib/prototype/backtest'
import { AIRLINES } from '../lib/prototype/apix-data'
...
export function Backtesting() {
  ...
  const result = useMemo<BacktestResult>(
    () => runBacktest(period.start, period.end, airlineCode),
    [period.start, period.end, airlineCode]
  )
          ...
          <Card className="p-4 md:p-6">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="ml-auto">
                <span className={cn('inline-flex items-center gap-1.5 rounded-sm border px-3 py-1.5 text-xs font-semibold', statusColors[result.status])}>
                  {result.status === 'Completed' ? <CheckCircle2 className="h-3.5 w-3.5" aria-hidden="true" /> : <AlertTriangle className="h-3.5 w-3.5" aria-hidden="true" />}
                  {result.status}
                </span>
              </div>
            </div>
            ...
            <dl className="mt-6 grid gap-px overflow-hidden rounded-sm border border-border bg-border sm:grid-cols-3 lg:grid-cols-6">
              {[
                ['Reference window', `${formatDate(result.start)} – ${formatDate(result.end)}`],
                ['Base period', BASE_PERIOD],
                ['Days evaluated', `${result.availableDays} of ${result.expectedDays}`],
                ['Observations', formatNumber(result.observations)],
                ['Average index', result.average.toFixed(2)],
                ['Route coverage', `${result.routes} routes · ${result.airlines} ${result.airlines === 1 ? 'carrier' : 'carriers'}`],
              ].map(([term, value]) => (
